...
class HeatEquationSolver(object):
    ...
    def train_combine(self, X, X_boundary, \
        ...
        print(timespace)
        all_t      = np.repeat(timespace, len(vis_points), 0).reshape((-1, 1)) # Test on timestride
        print(all_t.shape, meshX.shape)
        # all_t = np.ones((len(meshX.reshape((-1, 1))), 1)) # Test on t = 1
        all_points = np.concatenate([all_t, all_points[:, 0].reshape((-1, 1)), all_points[:, 1].reshape((-1, 1))], axis=1)
        # Training
        ls_boundary  = []
        ls_inner     = []
        ls_l2        = []
